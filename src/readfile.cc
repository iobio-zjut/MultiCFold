...
double fun_extract_contact(double **arr, int seq_length, double &number, int &flick_on_off, string path, string contacttxtpath)//  read contact
{
    string line;
    int rsd_start,rsd_end;
    double contact_p, rsd_starttemp, rsd_endtemp;
    rsd_start = rsd_end = 0;
    contact_p = rsd_starttemp = rsd_endtemp = 0.0;
    
    ifstream myfile1(path.c_str());  
    
    if(myfile1.is_open())
      std::cout<<"contactfile open success!!!"<<std::endl;
    else
      std::cout<<"contactfile open failed!!!!"<<std::endl;
    ofstream contactTxt;
    contactTxt.open(contacttxtpath.c_str());
	std::cout<<"contactTxt open success!!!!"<<std::endl;
    double contact_timer=0;
    double discontact_timer=0;
    double contact_blend=1;
    while(getline (myfile1, line))
    {
		istringstream is(line);

		is>>rsd_starttemp>>rsd_endtemp>>contact_p;
		std::cout<<"rsd_starttemp:"<<rsd_starttemp<<"rsd_endtemp:"<<rsd_endtemp<<"contact_p:"<<contact_p<<std::endl;
		rsd_start = ceil(rsd_starttemp);
		rsd_end = ceil(rsd_endtemp);
		std::cout<<"rsd_start:"<<rsd_start<<"rsd_end:"<<rsd_end<<std::endl;
		if( fabs(rsd_end - rsd_start) >= 6){
			std::cout<<"arr[rsd_start-1][rsd_end-1]_A:"<<arr[rsd_start-1][rsd_end-1]<<std::endl;
			arr[rsd_start-1][rsd_end-1] = contact_p;
			std::cout<<"arr[rsd_start-1][rsd_end-1]_B:"<<arr[rsd_start-1][rsd_end-1]<<std::endl;
			if(contact_p >= 0.5){	      
			  contactTxt<<std::setw(5)<<rsd_start<<"\t"<<std::setw(5)<<rsd_end<<"\t"<<std::setw(5)<<arr[rsd_start-1][rsd_end-1]<<std::endl;		      
			  contact_timer = contact_timer + 1;	      
			}
			if(contact_p <= 0.1){	
			  contactTxt<<std::setw(5)<<rsd_start<<"\t"<<std::setw(5)<<rsd_end<<"\t"<<std::setw(5)<<arr[rsd_start-1][rsd_end-1]<<std::endl;		
			  discontact_timer = discontact_timer + 1;
			}
		}
    }
    ...
    contact_blend = contact_timer/(contact_timer+discontact_timer);
    number = contact_timer;
    
    contactTxt<<"\n"<<std::endl;
    contactTxt<<"flick_on_off:\t"<<std::setw(5)<<flick_on_off<<"contact_blend:\t"<<std::setw(5)<<contact_blend<<std::endl;
    contactTxt<<"contact_timer:\t"<<std::setw(5)<<contact_timer<<"\tdiscontact_timer:\t"<<std::setw(5)<<discontact_timer<<"\tnumber:\t"<<std::setw(5)<<number<<std::endl;
    
    myfile1.close();
    myfile1.clear();	
    contactTxt.close();
    return contact_blend;  
}
