...
void ClassicAbinitio::apply( pose::Pose & pose ) {
	...
	const int halfpop_size = pop_size/2;
	...
	if(!false){
	  core::pose::Pose posetemp;
	  core::pose::Pose *newpose = new core::pose::Pose [pop_size];
	  core::pose::Pose *myposebetter = new core::pose::Pose [halfpop_size];
  
																	      
	  double **contact1 = new2DDoubleArr(seq_len, seq_len);
	  double **contact2 = new2DDoubleArr(seq_len, seq_len);
	  double **contact3 = new2DDoubleArr(seq_len, seq_len);
	  double **contact4 = new2DDoubleArr(seq_len, seq_len);
	  
	  double **distancemap = new2DDoubleArr(seq_len, seq_len);
	  double E1, E2, E3, E4, Etemp1, Etemp2, Etemp3, Etemp4, Esc1, Esc2, Esc3, Esc4;
	  double spaceAveEvalue,spaceStdEvalue,minAve,maxAve,minStd,maxStd;
	  
	  double blend_contact1,blend_contact2,blend_contact3,blend_contact4;
	  
	  E1 = E2 = E3 = E4 = Esc1 = Esc2 = Esc3 = Esc4 = Etemp1 = Etemp2 = Etemp3 = Etemp4 = minAve = maxAve = minStd = maxStd = spaceAveEvalue = spaceStdEvalue = 0.0;

	  int flick_on_off1,flick_on_off2,flick_on_off3,flick_on_off4;
	  ...
	  flick_on_off1 = flick_on_off2 = flick_on_off3 = flick_on_off4 = 0;
	  blend_contact1=fun_extract_contact(contact1, seq_len, num1, flick_on_off1,"TripletRes.txt","./TripletResContact");      // first contact	  
	  blend_contact2=fun_extract_contact(contact2, seq_len, num2, flick_on_off2,"PSIPRED.txt","./PSIPREDContact");
	  blend_contact3=fun_extract_contact(contact3, seq_len, num3, flick_on_off3,"RaptorX.txt","./RaptorXContact");
	  blend_contact4=fun_extract_contact(contact4, seq_len, num4, flick_on_off4,"SpotContact.txt","./SpotContactContact");
	  
	  if(flick_on_off1 == 1 && flick_on_off2 == 1 && flick_on_off3 == 1 && flick_on_off4 == 1){
	    blend_contact1 = blend_contact2 = blend_contact3 = blend_contact4 = -1;
	  }
	 ...
	 for(int i = 0;i<pop_size;i++){
		 ...
		 E1 = energyDesignByMHH(mypose[i], contact1, seq_len, blend_contact1);
		 E2 = energyDesignByMHH(mypose[i], contact2, seq_len, blend_contact2);
		 E3 = energyDesignByMHH(mypose[i], contact3, seq_len, blend_contact3);
		 E4 = energyDesignByMHH(mypose[i], contact4, seq_len, blend_contact4);
		 aveE = (weight1*E1+weight2*E2+weight3*E3+weight4*E4)/(weight1+weight2+weight3+weight4);

		 stdE = sqrt(( pow((E1 - aveE), 2) + pow((E2 - aveE), 2) + pow((E3 - aveE), 2) + pow((E4 - aveE), 2))/4);
		 cout<<"aveE = "<<aveE<<std::endl;
		 cout<<"stdE = "<<stdE<<std::endl;
		 if(i == 0){
			 minAve = maxAve = aveE;
			 minStd = maxStd = stdE;
		 }
	 ...
	 spaceAveEvalue = maxAve - minAve;
	 spaceStdEvalue = maxStd - minStd;
	 cout<<"spaceAveEvalue = "<<spaceAveEvalue<<std::endl;
	 cout<<"spaceStdEvalue = "<<spaceStdEvalue<<std::endl;
	 
	 aftermerge_sort(mypose, 0, pop_size-1,newpose,seq_len,contact1,contact2,contact3,contact4,spaceAveEvalue,spaceStdEvalue,minAve,minStd,blend_contact1,blend_contact2,blend_contact3,blend_contact4,weight1,weight2,weight3,weight4);
	 
	  for(int i = 0; i < halfpop_size; i++)
	      myposebetter[i] = mypose[i];                              
	  
	  core::pose::Pose Ctrial, Ctrial_frag, Etotal_lowest_pose ;
	  double Etotal_lowest = 0.0;
	  bool success(false);
	  bool exchange(false);
	  int count_frag = 0;
	  int gmax = 500;
	  ...
	  Etotal_lowest_pose = myposebetter[0];
	  std::cout<<"Etotal_lowest_pose"<<'\t'<<Etotal_lowest_pose<<std::endl;
	  
	  for(int g = 0; g < gmax; g++){
	    for(int m = 0; m < halfpop_size; m++){

	      double pCR = 0.0;
	      pCR = rand()%100/(double)101;
		  mymethod.convex_mutation1(myposebetter, Ctrial,halfpop_size, seq_len, m);

	      while(!success && count_frag < 1000){
			Ctrial_frag = Ctrial;
			if(g < gmax * 0.2)
			...
			if((*score_stage4_)(Ctrial_frag) < (*score_stage4_)(myposebetter[halfpop_size-1])){
				Ctrial = Ctrial_frag;
				success = true;
				std::cout<<"after cycle  "<<count_frag<<"   accept fragment assemble"<<std::endl;
		  
			}
			else if( whether_accept_new( (*score_stage4_)(Ctrial_frag), (*score_stage4_)(myposebetter[halfpop_size-1])) ){
				Ctrial = Ctrial_frag;
				success = true;
				std::cout<<"after cycle  "<<count_frag<<"   accept fragment assemble"<<std::endl;
		  		}
		...
		if(success)
			mypose[halfpop_size+m] = Ctrial;
		else
			mypose[halfpop_size+m] = myposebetter[m];
	  ...
	  myposebetter[0].dump_pdb("model.pdb");
	  pose = myposebetter[0];
	  
	  
	  E1 = energyDesignByMHH(pose, contact1, seq_len, blend_contact1);
	  E2 = energyDesignByMHH(pose, contact2, seq_len, blend_contact2);
	  E3 = energyDesignByMHH(pose, contact3, seq_len, blend_contact3);
	  E4 = energyDesignByMHH(pose, contact4, seq_len, blend_contact4);
	 
	  ofstream information;
	  information.open("./information.txt");	
	  
	  information<<std::setw(5)<<"E1"<<std::setw(5)<<"E2"<<std::setw(5)<<"E3"<<std::setw(5)<<"E4"<<std::setw(5)<<"Escore"<<std::setw(5)<<"RMSD"<<std::endl;
	  ...
	  release2DArr(seq_len, contact3);
	  release2DArr(seq_len, contact4);
	  release2DArr(seq_len, distancemap);
	  delete []mypose;
...
void ClassicAbinitio::aftermerge(core::pose::Pose *mypose,int start,int end, core::pose::Pose newpose[], int seq_len,double **contact1, double **contact2, double **contact3,double **contact4,double spaceAveEvalue,double spaceStdEvalue,double minAve,double minStd,double blend_contact1,double blend_contact2,double blend_contact3,double blend_contact4,double weight1,double weight2,double weight3,double weight4){
    ...
    {
        if(normalizationAveandStd(energyDesignByMHH(mypose[left_index], contact1, seq_len, blend_contact1), energyDesignByMHH(mypose[left_index], contact2, seq_len, blend_contact2), energyDesignByMHH(mypose[left_index], contact3, seq_len, blend_contact3), energyDesignByMHH(mypose[left_index], contact4, seq_len, blend_contact4),spaceAveEvalue,spaceStdEvalue,minAve,minStd,weight1,weight2,weight3,weight4)
			<= normalizationAveandStd(energyDesignByMHH(mypose[right_index], contact1, seq_len, blend_contact1), energyDesignByMHH(mypose[right_index], contact2, seq_len, blend_contact2), energyDesignByMHH(mypose[right_index], contact3, seq_len, blend_contact3), energyDesignByMHH(mypose[right_index], contact4, seq_len, blend_contact4),spaceAveEvalue,spaceStdEvalue,minAve,minStd,weight1,weight2,weight3,weight4))
	
            newpose[result_index++] = mypose[left_index++];
	
        else
            newpose[result_index++] = mypose[right_index++];
    }
...
void ClassicAbinitio::aftermerge_sort(core::pose::Pose *mypose,int start,int end, core::pose::Pose newpose[], int seq_len,double **contact1, double **contact2, double **contact3,double **contact4,double spaceAveEvalue,double spaceStdEvalue,double minAve,double minStd,double blend_contact1,double blend_contact2,double blend_contact3,double blend_contact4,double weight1,double weight2,double weight3,double weight4){
    if(1 == end - start)   //last only two elements
    {
        if(normalizationAveandStd(energyDesignByMHH(mypose[start], contact1, seq_len, blend_contact1), energyDesignByMHH(mypose[start], contact2, seq_len, blend_contact2), energyDesignByMHH(mypose[start], contact3, seq_len, blend_contact3), energyDesignByMHH(mypose[start], contact4, seq_len, blend_contact4),spaceAveEvalue,spaceStdEvalue,minAve,minStd,weight1,weight2,weight3,weight4)
			> normalizationAveandStd(energyDesignByMHH(mypose[end], contact1, seq_len, blend_contact1), energyDesignByMHH(mypose[end], contact2, seq_len, blend_contact2), energyDesignByMHH(mypose[end], contact3, seq_len, blend_contact3), energyDesignByMHH(mypose[end], contact4, seq_len, blend_contact4),spaceAveEvalue,spaceStdEvalue,minAve,minStd,weight1,weight2,weight3,weight4))
        {
            core::pose::Pose temp = mypose[start];
            mypose[start] = mypose[end];
            mypose[end] = temp;
        }
        return;
    }
        ...
        aftermerge_sort(mypose, start, (end - start + 1) / 2 + start, newpose, seq_len,contact1,contact2,contact3,contact4,spaceAveEvalue,spaceStdEvalue,minAve,minStd,blend_contact1,blend_contact2,blend_contact3,blend_contact4,weight1,weight2,weight3,weight4);
        aftermerge_sort(mypose, (end - start + 1) / 2 + start + 1, end, newpose, seq_len,contact1,contact2,contact3,contact4,spaceAveEvalue,spaceStdEvalue,minAve,minStd,blend_contact1,blend_contact2,blend_contact3,blend_contact4,weight1,weight2,weight3,weight4);
        aftermerge(mypose, start, end, newpose,seq_len,contact1,contact2,contact3,contact4,spaceAveEvalue,spaceStdEvalue,minAve,minStd,blend_contact1,blend_contact2,blend_contact3,blend_contact4,weight1,weight2,weight3,weight4);
...
double ClassicAbinitio::normalizationAveandStd(double E1, double E2, double E3, double E4,double spaceAveEvalue,double spaceStdEvalue,double minAve,double minStd,double weight1,double weight2,double weight3,double weight4){
    
    double aveE, stdE, sum ,normalizationAve,normalizationStd;
    ...
    normalizationAve = (aveE - minAve)/spaceAveEvalue;
    normalizationStd = (stdE - minStd) /spaceStdEvalue;
    
    cout<<"normalizationAve = "<<normalizationAve<<std::endl;
    cout<<"normalizationStd = "<<normalizationStd<<std::endl;
    
	sum = 0.75*normalizationAve + 0.25*normalizationStd;
    return sum;
}

double ClassicAbinitio::energyDesignByMHH(core::pose::Pose decoy, double **contact, int seq_length, double blend_contact){

  using namespace core;
  using namespace scoring;
  using namespace core::scoring;
  double sumEct = 0.0;
  double temp_contact = 0.0;
  double temp_discontact = 0.0;
      for(int i = 1; i < seq_length + 1; i++){
		  for(int j = i; j < seq_length + 1; j++){ 
			if(blend_contact < 0){
				if(contact[i - 1][j - 1] < 0.1 && abs(i - j) >= 6){
				  temp_discontact = core::scoring::residues_distance_CB(decoy, i, j);
				  if(temp_discontact <= 8){
					sumEct += (8 - temp_discontact)*(exp(1 - contact[i - 1][j - 1]));
				  }
				  else if(temp_discontact > 8 && temp_discontact <=10){
					sumEct += (8 - temp_discontact)*log(2 - contact[i - 1][j - 1]);
				  }
				  else{	      
					sumEct += (8 - temp_discontact)*(exp(1 - contact[i - 1][j - 1])-1);
				  }
				}
			}else{
				if(contact[i - 1][j - 1] >= 0.5 && abs(i - j) >= 6){
					temp_contact = core::scoring::residues_distance_CB(decoy, i, j);
					if(temp_contact <= 8){
					  sumEct += 2*(1-blend_contact)*(temp_contact-8)*(exp( contact[i - 1][j - 1]));
					}
					else if(temp_contact > 8 && temp_contact <=10){
					  sumEct += 2*(1-blend_contact)*(temp_contact - 8)*log(1 + contact[i - 1][j - 1]);
					}
					else{	      
					  sumEct += 2*(1-blend_contact)*(temp_contact - 8)*(exp( contact[i - 1][j - 1] )-1);
					}
				}
			  if(contact[i - 1][j - 1] < 0.1 && abs(i - j) >= 6){
				temp_discontact = core::scoring::residues_distance_CB(decoy, i, j);
				if(temp_discontact <= 8){
				  sumEct += blend_contact*(8 - temp_discontact)*(exp(1 - contact[i - 1][j - 1]));
				}
			  }
			}		
		  }
  ...
  sumEct = sumEct + (*score_stage4_)(decoy);
  cout<<"sumEct = "<<sumEct<<std::endl;
